...
\parbox{13cm}{	
	...
	\frac{B^2}{\mu_0} \cdot A \text{ (für } \mu_{rFe} \gg  \text{ 1)}}$$ }
...
und sind immer geschlossen.\\ 
	\begin{tabular}[c]{ | p{5cm} | p{8cm} | p{4cm} | }
		...
		\hline
		Permeabilitätskonstante & $\mu_0 = 4 \pi \cdot 10^{-7}$ & $\frac{Vs}{Am}$\\
		\hline
		Permeabilität
		...
		\vec{B}$ (Material unabhängig) & $\frac{Vs}{m^2} = T$ (Tesla) \\
		\hline
		Lorentzkraft
...
\subsubsection{Ohne Luftspalt}
  ...
  \end{tabular}
	...
	$l_M=\mu_Ml_L$ \\
	$L=\frac{N_1 \cdot \Theta_1}{I_1}=\frac{N_1^2}{R_M}$
\end{multicols}
...
\subsubsection{Mit Luftspalt}
  ...
  Der Fluss im Luftspalt $\Phi_L $ ändert dann zu $
...
\mu_0 \mu_r \cdot A_L }{l_L} \qquad H^*=\frac{\Theta}{l_{Fe}} \qquad
B^*=\frac{\Phi_k}{A_{Fe}}=\frac{\Theta \cdot \mu_0 \mu_r \cdot A_L }{l_L
A_{Fe}}=\overbrace{\frac{\mu_0 \mu_r \cdot \Theta}{l_L}}^{A_L=A_{Fe}}$$ 
